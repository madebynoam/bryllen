...
4. Swap the npm package in the consumer project to local:
   ...
   # Create package.json if it doesn't exist yet (prevents /canvai-new from installing from GitHub)
   ...
   Creating `package.json` first is critical: without it, `/canvai-new` will install canvai from GitHub (hardcoded in the skill), overwriting the local swap.
...
6. Tell the user:
   ...
   - Test with `/canvai-new <project-name>`
